        ...
        <tbody>
            <tr>
                @foreach($attributesArray as $attributes)
                    <td onclick="location.href='/results/kgv/epg/{{$attributes['ID_EVENTO']}}';" style="cursor: pointer; width: 25%; padding: 10px;">
                        <div>
                            ...
                            <a style="font-size: 12px" href='{{ $attributes['link'] }}'>{{ $attributes['link_text'] }}</a>
                        </div>
                    </td>
                    @if($loop->iteration % 4 == 0)
                        </tr><tr> <!-- Quebra a linha a cada 4 células -->
                    @endif
                @endforeach
            </tr>
        </table>
